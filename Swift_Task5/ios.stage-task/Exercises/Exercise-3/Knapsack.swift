...
public final class Knapsack {
    ...
    func findMaxKilometres() -> Int {
        ...
        let drinksCombinations = getCombinations(maxWeight: maxWeight,
                                                 itemWeight: drinksWeight,
                                                 itemValue: drinksValue)
        let foodsCombinations = getCombinations(maxWeight: maxWeight,
                                                 itemWeight: foodsWeight,
                                                 itemValue: foodsValue)
        
        for i in 0...maxWeight {
            if min(drinksCombinations[i], foodsCombinations[maxWeight - i]) > maxKilometres {
                maxKilometres = min(drinksCombinations[i], foodsCombinations[maxWeight - i])
            }
        }
    ...
    func getCombinations(maxWeight: Int, itemWeight: [Int], itemValue: [Int]) -> [Int] {
